import { Browser, DomUtil, Point, DivIcon, Marker, Polyline, Polygon, DomEvent, LayerGroup, Control, Util } from 'leaflet';
...
var CLICK_EVT = Browser.mobile ? 'touchend' : 'click';
...
function drawingPaneContainer(options) {
  ...
  var drawPane = DomUtil.create('div', cls('leaflet-map-overlay-pane', "leaflet-pane" + (active ? '' : ' inactive') + (fadeOnActivation ? ' fading-enabled' : '')));
  return drawPane;
}
...
function addEndClickArea(control, _ref) {
  ...
  var marker = DomUtil.create('div', cls('end-selection-area'), pane);
...
function onAddPoint(event) {
  ...
  var marker = new Marker(map.containerPointToLatLng(point), {
    icon: icon,
    draggable: true
  });
...
function onAddMarker(_ref) {
  ...
  if (this.phase === 'draw') {
    ...
    if (enoughPoints) {
      this.closeLine = new Polyline([markers[0].marker.getLatLng(), markers[markers.length - 1].marker.getLatLng()], {
        weight: 3,
        color: '#c0c0c0',
...
function onUpdatePolygon() {
  ...
  var markers = this.markers;
  var enoughPoints = markers.length >= 3;
  var polygon = new Polygon(markers.map(function (_ref5) {
...
function onGhostMarkerDrag(index) {
  ...
  return function (event) {
    ...
    requestAnimationFrame(function () {
      ...
      _this5.ghostPolygon = new Polygon([firstPoint.marker.getLatLng(), latlng, lastPoint.marker.getLatLng()], {
...
function onMouseMove(event) {
  if (!this.mapMoving && this.markers.length === 0 && event.which !== 0 && event.buttons === 1) {
    ...
    if (!this.rectDrawing) {
      ...
      this.draggingRect = new LayerGroup().addLayer(new Polyline([], props)).addLayer(new Polyline([], props)).addTo(map);
...
var DrawAreaSelection = Control.extend({
  options: {
    active: false,
  ...
  addUserHelpPanel: function addUserHelpPanel(pane) {
    var panel = DomUtil.create('div', cls('draw-pane-help'));
    panel.setAttribute('id', 'draw-panel-help');
    panel.setAttribute('role', 'tooltip');
    var help = "Define a polygon by clicking of the map to define vertexes" + (Browser.mobile ? "." : " or click-and-drag to obtain a rectangular shape.");
